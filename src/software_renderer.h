...
class SoftwareRendererImp : public SoftwareRenderer {
 public:
  SoftwareRendererImp(SoftwareRendererRef *ref = NULL)
      : SoftwareRenderer(), ref(ref) {}
...
class SoftwareRendererRef : public SoftwareRenderer {
 public:
  ...
  // rasterize a line
  void rasterize_line_helper(float x0, float y0, float x1, float y1, int width,
                             int height, Color color, SoftwareRendererImp *imp);
 ...
 private:
  ...
  // rasterize a triangle
  void rasterize_triangle(float x0, float y0, float x1, float y1, float x2,
                          float y2, Color color, void *thread_data = NULL);
