...
void DrawSVG::init() {
  ...
  software_renderer_imp =
      new SoftwareRendererImp((SoftwareRendererRef*)software_renderer_ref);
...
void DrawSVG::draw_diff() {
  ...
  for (size_t i = 0; i < width * height; i++) {
    ...
    if (normalize_diff)
      max_error = max(max_error,
                      (float)framebuffer[i * 4 + 0] * framebuffer[i * 4 + 0] +
                          framebuffer[i * 4 + 1] * framebuffer[i * 4 + 1] +
                          framebuffer[i * 4 + 2] * framebuffer[i * 4 + 2]);
  ...
  if (normalize_diff && max_error > 0) {
    ...
    for (size_t i = 0; i < width * height; i++) {
      framebuffer[i * 4 + 0] =
          (float)framebuffer[i * 4 + 0] / max_error * 255.f;
      framebuffer[i * 4 + 1] =
          (float)framebuffer[i * 4 + 1] / max_error * 255.f;
      framebuffer[i * 4 + 2] =
          (float)framebuffer[i * 4 + 2] / max_error * 255.f;
    }
  }
}
...
void DrawSVG::draw_zoom() {
  ...
  size_t cX =
      max(regionSize / 2, min(width - regionSize / 2 - 1, (size_t)cursor_x));
  size_t cY = max(regionSize / 2,
                  min(height - regionSize / 2 - 1, height - (size_t)cursor_y));
...
void DrawSVG::redraw() {
  ...
  Matrix3x3 m_imp =
      norm_to_screen * viewport_imp[current_tab]->get_canvas_to_norm();
  Matrix3x3 m_ref =
      norm_to_screen * viewport_ref[current_tab]->get_canvas_to_norm();
